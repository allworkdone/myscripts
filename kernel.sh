...
clone() {
	...
	if [[ $COMPILER == "clang" ]]; then
		# Clone STRIX Clang
		git clone --depth=1 https://gitlab.com/fiqri19102002/strix-clang -b lto-pgo/14.x clang
...
compile() {
	...
	tg_post_msg "<b>• liquid° Kernel •</b>%0Astarted on: <code>liquidCI</code>%0Adevice: <b>Mi A3</b>%0Abranch: <code>$(git rev-parse --abbrev-ref HEAD)</code>%0Acompiler: <code>${KBUILD_COMPILER_STRING}</code>%0Astart date: <code>$(date)</code>%0A<b>Status : </b>#Personal"
